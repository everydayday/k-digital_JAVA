...
//			System.out.println("newNode data : " + newNode.data + "q.data: "+ q.data);
	...
	public boolean Search(SimpleObject5 element, Comparator<SimpleObject5> cc) { 
		// 전체 리스트를 올림차순 순서대로 출력한다. => add 가 오름차순으로 추가하니 정렬 안 해도 되지 않을까
//		Node2 q, current = first;
//		q = current;
		Node2 q = first;
		Node2 current = new Node2(element);
		
		while(q != null) {
			System.out.println(q.data);
			if(cc.compare(q.data, current.data ) == 0) return true;
			q = q.link;
		}
...
public class 실습9_2객체연결리스트_test {
	...
	enum Menu {
		...
		static Menu MenuAt(int idx) {                // 순서가 idx번째인 열거를 반환
			for (Menu m : Menu.values())
				if (m.ordinal() == idx)				// ordinal : position
					return m;
			return null;
		}
	...
	static Menu SelectMenu() {
		...
		do {
			...
			System.out.print(" : ");
			key = sc.nextInt();
		} while (key < Menu.Add.ordinal() ||key > Menu.Exit.ordinal());	// 반대로 되어야 하는거 아닌가

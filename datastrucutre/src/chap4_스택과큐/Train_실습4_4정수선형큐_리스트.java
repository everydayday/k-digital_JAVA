...
//--- 큐에서 데이터를 디큐 ---//
	public int deque() throws EmptyQueueException {
		...
		// 제거 후 값 반환
		int temp = que.remove(front);
		// list 는 저절로 배열 줄이므로 front는 가만히 있고 rear을 줄여야 한다
		rear --;
		return temp;
	}
...
//--- 큐를 비움 --- peek() 처럼 예외 발생 구현//
	public void clear() throws EmptyQueueException {
		 ...
		 */
		if (isEmpty()) // queue이 빔
			throw new EmptyQueueException("clear is empty");
		que.removeAll(que);
		front = rear = 0;
...
//--- 큐에서 x를 검색하여 인덱스(찾지 못하면 –1)를 반환 ---//
	...
	public void dump() throws EmptyQueueException  {
		if(isEmpty()) throw new EmptyQueueException("dump is empty");

...
//			System.out.println("count :" + count);
			...
			if(iy != -1) { // push 할 때				
				...
				ix ++; iy = 0; // 다음 행은 iy는 0에서 부터 시작				
				if(count != 8) continue;
			}			
		
			if(iy == -1 || count == 8 ) {	// pop이 필요할 때 : 존재하지않는다 or 가득 참
				
				if(count == 8) {	 // 가득 찼을 때 
					numberSolutions ++;
					System.out.println("numberSolutions :"+ numberSolutions);
					showQueens(d);
				}
				try {
					p = st.pop();
